 ...
 * แต่ตรวจความถูกต้องทางคณิตศาสตร์ของเนื้อหาไม่ได้
 * — ส่วนนั้นยังต้องให้คนอ่าน
 */
...
// -----------------------------------------------------------------------------
// Type guard สำหรับ numeric block
//
// numeric อาจมีอยู่จริงในข้อมูล content แต่ยังไม่ได้ถูกประกาศไว้ใน union type
// ของ schema.ts ทำให้ TypeScript มอง b.kind === "numeric" เป็นไปไม่ได้
//
// เราจึงตรวจจากโครงสร้างจริงของ object แทน
// -----------------------------------------------------------------------------
...
function isNumericBlock(value: unknown): value is NumericBlock {
...
// -----------------------------------------------------------------------------
// 2. topicId ที่อ้างถึงต้องมีอยู่จริง
//    และ chapterId ของหัวข้อก็ต้องมีอยู่จริง
// -----------------------------------------------------------------------------
...
for (const l of lessons) {
  for (const s of l.sections) {
    for (const b of s.blocks) {
      ...
      // numeric
      //
      // ใช้ type guard แทน b.kind === "numeric"
      // เพื่อหลีกเลี่ยงปัญหา TypeScript ที่มอง numeric เป็น never
      // -----------------------------------------------------------------------
      ...
      if (correct !== 1) {
        err(
          `${l.slug}/${s.id}: ควิซมีคำตอบถูก ${correct} ข้อ (ต้องมี 1 ข้อพอดี) — "${b.prompt.slice(0, 40)}…"`,
        );
      }
...
function checkDollars(
  ...
  if (count % 2 !== 0) {
    err(
      `${where}: เครื่องหมาย $ ไม่เป็นคู่ — "${text.slice(0, 50)}…"`,
    );
  }
}
...
for (const l of lessons) {
  for (const s of l.sections) {
    for (const b of s.blocks) {
      ...
      // numeric ต้องใช้ type guard เช่นเดียวกัน
      if (isNumericBlock(b)) {
        checkDollars(b.prompt, where);
        checkDollars(b.explain, where);
...
// -----------------------------------------------------------------------------
// 9. ช่องที่เรนเดอร์เป็นสูตรล้วนต้องไม่มี $ คั่น
//
// ใส่ $ ลงไปจะทำให้ KaTeX พังทั้งก้อน
...
function checkRawLatex(
  ...
  if (latex.includes("$")) {
    err(
      `${where}: ${field} เป็นสูตรล้วนอยู่แล้ว ห้ามใส่ $ — "${latex.slice(0, 50)}…"`,
    );
  }
}
...
//
// ตัวเลขนี้มีไว้เตือนว่าถ้าวันหนึ่งมีคนถอดการสลับออก
    ...
    for (const s of l.sections) {
      for (const b of s.blocks) {
        ...
        if (GIVEAWAY.test(first.trim())) {
          warn(
            `${l.slug}/${s.id}: แนวทางขั้นแรกออกแนวบอกวิธีทำ — "${first.slice(0, 40)}…"`,
          );
        }
